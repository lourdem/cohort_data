...
def sort_by_cohort(filename):
    ...
    for line in cohort_data:
        ...
        order = line.split('|')
        # if "Winter" in line:
        #     winter_16.append(order[0])    
        if order[4] == "Winter 2016":
            winter_16.append(order[0])
        if order[4] == "Spring 2016":
            spring_16.append(order[0])
        if order[4] == "Summer 2016":
            summer_16.append(order[0])
        if order[4] == "Fall 2015":
            fall_15.append(order[0])
        if order[4] == "G":
            ghosts.append(order[0])  
            # turn names into strings and then append                 
